...
class Pyramid(nn.Module):
    def __init__(self):
        ...
        self.exit3 = nn.Conv2d(256, 3, kernel_size=3, stride=1, padding=1)
        self.exit2 = nn.Conv2d(32, 3, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        up3 = self.tunnel3(lv3)
        lv3 = self.up3(up3)

        up2 = self.tunnel2((self.con2(torch.cat([lv2, lv3], 1)), lv3))[0] + lv3
        lv2 = self.up2(up2)

        lv1 = self.tunnel1((self.con1(torch.cat([lv1, lv2], 1)), lv2))[0] + lv2
        return self.exit3(up3), self.exit2(up2), self.exit(lv1)
