...
class Pyramid(nn.Module):
    def __init__(self):
        ...
        self.up3 = nn.Sequential(nn.Conv2d(256, 32 * 4, 3, 1, 1, bias=False),
                                 nn.PixelShuffle(2),
                                 nn.ReLU(inplace=True))
        self.con2 = nn.Conv2d(160 + 64, 128, 5, 1, 2, bias=False)

        self.up2 = nn.Sequential(nn.Conv2d(128, 32 * 4, 3, 1, 1, bias=False),
                                 nn.PixelShuffle(2),
                                 nn.ReLU(inplace=True))
        self.con1 = nn.Conv2d(96 + 64, 64, 5, 1, 2, bias=False)

        tunnel = [DResNeXtBottleneck() for _ in range(30)]
        self.tunnel3 = nn.Sequential(*tunnel)

        depth = 3
        tunnel = [ResNeXtBottleneck(128 + 160 + 64, 128, cardinality=16, dilate=1) for _ in range(depth)]
        tunnel += [ResNeXtBottleneck(128 + 160 + 64, 128, cardinality=16, dilate=2) for _ in range(depth)]
        tunnel += [ResNeXtBottleneck(128 + 160 + 64, 128, cardinality=16, dilate=4) for _ in range(depth)]
        tunnel += [ResNeXtBottleneck(128 + 160 + 64, 128, cardinality=16, dilate=2),
                   ResNeXtBottleneck(128 + 160 + 64, 128, cardinality=16, dilate=1)]
        self.tunnel2 = nn.Sequential(*tunnel)

        tunnel = [ResNeXtBottleneck(64 + 32 * 3 + 64, 64, cardinality=8, dilate=1) for _ in range(depth)]
        tunnel += [ResNeXtBottleneck(64 + 32 * 3 + 64, 64, cardinality=8, dilate=2) for _ in range(depth)]
        tunnel += [ResNeXtBottleneck(64 + 32 * 3 + 64, 64, cardinality=8, dilate=4) for _ in range(depth)]
        tunnel += [ResNeXtBottleneck(64 + 32 * 3 + 64, 64, cardinality=8, dilate=8) for _ in range(depth)]
        tunnel += [ResNeXtBottleneck(64 + 32 * 3 + 64, 64, cardinality=8, dilate=2),
                   ResNeXtBottleneck(64 + 32 * 3 + 64, 64, cardinality=8, dilate=1)]
    ...
    def forward(self, x):
        ...
        lv3_ori = self.up3(lv3)
        lv3 = self.up3(self.tunnel3(lv3))
        lv3_res = lv3 - lv3_ori

        features = torch.cat([lv2, lv3, lv3_ori, lv3_res], 1)
        lv2_res = self.up2(self.tunnel2((self.con2(features), features))[0])
        lv2_ori = self.up2(lv2)
        lv2 = lv2_ori + lv2_res

        features = torch.cat([lv1, lv2, lv2_ori, lv2_res], 1)
        lv1 = self.tunnel1((self.con1(features), features))[0] + lv1
