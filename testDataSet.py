...
parser.add_argument('--id', type=str, default=None, help='experiment identification')
...
if opt.cuda:
    netG.cuda()
try:
    os.makedirs(os.path.join(os.path.split(opt.dataroot)[0], os.path.split(opt.dataroot)[1] + 'result' + opt.id))
except OSError:
    pass
...
for input, name in dataloader_test:
    ...
    vutils.save_image(torch.squeeze(prediction.data).mul(0.5).add(0.5),
                      os.path.join(os.path.split(opt.dataroot)[0], os.path.split(opt.dataroot)[1] + 'result' + opt.id,
                                   name[0]),
                      padding=0)
    print(name[0] + '　処理完了')
    prediction = 0
